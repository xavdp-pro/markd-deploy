...
interface SchemaTreeProps {
  ...
  onExpandAll?: () => void;
  onCollapseAll?: () => void;
  onSelect: (schema: SchemaItem, event?: React.MouseEvent) => void;
  onSelectAll?: () => void;
    ...
    >
      ...
      {node.id !== 'root' && (
        <>
          ...
          {node.type === 'schema' && node.locked_by && onUnlock && (() => {
      ...
      >
        ...
        >
          {node.type === 'folder' ? (
            ...
            <Network size={16} className="text-blue-600 dark:text-blue-400 flex-shrink-0" />
          )}
